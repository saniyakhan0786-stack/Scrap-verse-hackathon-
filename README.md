# Scrap-verse-hackathon-
I selected a suitable sales dataset and explored its structure by examining the columns, data types, and overall data quality. This helped me understand the dataset before starting the cleaning and analysis process.
# smart product price tracker 
I bride data scraper studio project that turns publicly available product pages into structure product data and displays the result in a simple browser dashboard 
# how is capac studio was used 
this section is fine too lake that mostly as a once a sexually turn side the heal-demo.sh, but the real collector ID for one line that would be break or hill down.
Uses Bright Data Scraper Studio to create and run a custom scraper.
- Extracts product name, price, availability, rating, discount, and URL when available.
- Returns structured JSON data.
- Displays the structured results in `viewer/index.html`.
- Demonstrates Bright Data's self-healing workflow with `bdata scraper heal`.

## Architecture

Public product page
→ Bright Data Scraper Studio
→ Collector ID
→ Structured JSON
→ Simple dashboard

## Requirements

- Bright Data account
- Bright Data CLI
- A Bright Data Scraper Studio Collector ID
- A public target page that is allowed to be scraped
- ## Quick start

1. Log in to Bright Data:

```bash
npx -p @brightdata/cli bdata login
```

2. Create a scraper:

```bash
npx -p @brightdata/cli bdata scraper create "<TARGET_URL>" "Extract product name, price, currency, availability, rating, discount, and product URL as structured JSON."
```

